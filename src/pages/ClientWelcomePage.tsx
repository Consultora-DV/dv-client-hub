...
const TOTAL_STEPS = 5; // Pasos numerados (1..5). Paso 0 bienvenida, 6 final.
...
export default function ClientWelcomePage() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#0A0A0F] text-foreground relative overflow-hidden">
      ...
      {step > 0 && step < 6 && (
        <div
          className="fixed bottom-0 left-0 right-0 z-40 border-t border-border/30 bg-[#0A0A0F]/90 backdrop-blur-xl"
        ...
        >
          <div className="mx-auto max-w-lg px-5 py-4 flex items-center gap-3">
            ...
            <Button
              ...
              disabled={submitting}
              className="dv-shimmer-btn gold-gradient text-black font-semibold h-12 px-6 rounded-xl hover:opacity-95"
            >
              {submitting ? (
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center text-center min-h-[80vh] py-12">
      ...
      <motion.h1 {...stagger(2)} className="text-3xl md:text-4xl font-bold leading-tight max-w-md text-balance">
        Acabas de tomar una de las mejores decisiones para tu marca.
      </motion.h1>
      <motion.p {...stagger(3)} className="mt-4 text-muted-foreground max-w-sm">
        Este es el inicio de algo extraordinario. Tomémonos 3 minutos para conocernos.
      </motion.p>
      <motion.div {...stagger(4)} className="mt-8 flex items-center gap-2 text-sm text-primary">
        <Clock className="h-4 w-4 animate-pulse" />
        <span className="tracking-wide">5 pasos · 3 minutos</span>
      </motion.div>
...
function StepHeader({ num, title, subtitle }: { num: string; title: string; subtitle?: string }) {
  return (
    <div className="mb-8 mt-6">
      <div className="text-xs tracking-[0.3em] text-primary mb-2">{num}</div>
      ...
      <div className="space-y-6">
        ...
        <div className="pt-4">
          <Label className="text-sm text-muted-foreground">
            Nombre de tu marca o como quieres ser conocido *
          </Label>
          <Input
        ...
        {SECTORS.map((s, i) => {
          ...
          return (
            <motion.button
              ...
              className={cn(
                "relative h-28 rounded-xl border flex flex-col items-center justify-center gap-2 transition-all",
                active
...
function StepFinal({ email }: { email: string }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center text-center justify-center py-16 min-h-[80vh]">
      ...
      <motion.div
        ...
        className="mt-8 w-full max-w-sm glass gold-border rounded-2xl p-5 space-y-3 text-left"
      >
      ...
      <motion.a
        ...
        className="dv-shimmer-btn mt-8 inline-flex items-center gap-2 gold-gradient text-black font-semibold h-13 px-8 py-3 rounded-xl hover:opacity-95"
      >
