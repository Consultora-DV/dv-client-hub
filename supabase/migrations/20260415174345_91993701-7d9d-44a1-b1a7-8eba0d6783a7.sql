...
-- 1. Create enum
CREATE TYPE public.app_role AS ENUM ('admin', 'editor', 'diseñador', 'cliente');

-- 2. Create user_roles table first (has_role depends on it)
...
-- 3. Create has_role function (SECURITY DEFINER, no RLS recursion)
...
CREATE POLICY "Admins can update roles" ON public.user_roles FOR UPDATE TO authenticated USING (public.has_role(auth.uid(), 'admin'::app_role));
