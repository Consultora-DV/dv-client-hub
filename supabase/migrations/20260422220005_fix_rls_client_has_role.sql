...
-- Un usuario con rol admin/editor cuyo UUID coincida con un cliente_id podría acceder.
...
-- ============================================================
-- SCRIPTS (SELECT; UPDATE ya fue parchado en 20260422220002)
-- ============================================================
...
CREATE POLICY "Clients can view comments on own videos"
  ...
  USING (
    ...
    AND EXISTS (
      ...
      WHERE v.id = video_id AND v.cliente_id = auth.uid()
    )
  );
...
CREATE POLICY "Clients can view comments on own scripts"
  ...
  USING (
    ...
    AND EXISTS (
      ...
      WHERE s.id = script_comments.script_id AND s.cliente_id = auth.uid()
    )
  );
