...
-- La migración 20260423180233 dropó las tablas scripts y script_comments por error.
-- Este script las recrea con el schema correcto (sin columna date en script_comments,
-- índice sobre created_at) y con todas las RLS policies corregidas ya incorporadas.
...
-- scripts: equipo interno (acceso completo)
...
-- scripts: cliente (SELECT propio + UPDATE restringido a status aprobado/cambios)
...
-- script_comments: equipo interno (acceso completo)
...
-- script_comments: cliente (ver y agregar comentarios en sus propios guiones)
